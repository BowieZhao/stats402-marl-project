...
class Config:
    ...
    plan_bias_strength: float = 4.0    # back to working value (was 4 originally)
    ...
    R_capture: float = 10.0                  # collision with UNCAUGHT good
    ...
    R_encircle_per_extra: float = 4.0        # back to 4 (n=3→+8, n=4→+12)
    R_progress_first: float = 30.0           # killer of 1st-caught good
    R_progress_second: float = 40.0          # killer of 2nd-caught good
    R_progress_team_share: float = 15.0      # NEW: encircle-circle teammates get this
                                              # on a catch (split-incentive design)
    ...
    R_role_align: float = 1.0                # back to 1.0
    R_message_thrash_penalty: float = -3.0   # leader-only: unjustified message switch
    R_correct_message: float = 5.0           # back to 5 (was working)
